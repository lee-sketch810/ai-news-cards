...
> **자동 공백 채우기(Step 0 — Gap Check, 필수)**: 매 실행(스케줄 실행이든, 세션에서 사이트를 띄우는 경우든) 시작 시 `public/data/index.json`의 최신 에디션 날짜를 확인하고, 그 다음날부터 어제까지 발행 안 된 날짜가 있으면 **묻지 않고 자동으로** 1~11단계를 그 날짜들에 대해 먼저 실행한 뒤 오늘치를 처리한다. 스케줄이 앱 미실행·권한 미승인 등으로 하루 이상 건너뛰어도 다음 실행이 스스로 따라잡는다(self-healing). 오늘 날짜는 당일 로f업이 아직 미발행일 수 있으므로 검증 실패 시 정직하게 비워두고 다음 실행에서 채운다.
